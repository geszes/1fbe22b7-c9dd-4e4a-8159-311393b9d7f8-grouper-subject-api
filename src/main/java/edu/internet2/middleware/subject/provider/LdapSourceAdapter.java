...
import edu.internet2.middleware.grouperconfigbase.ExtractedUtils;
    ...
    private Iterator<SearchResult> getLdapResultsHelper(Search search, String searchValue, String[] attributeNames, boolean firstPageOnly ) {
      ...
      if (subjectStatusQuery) {
        ...
        if ((cp=searchValue.indexOf(',')) >0 ) {
            ...
            if (last!=null) filter = ExtractedUtils.replace(filter, "%LAST%", escapeSearchFilter(last));
            if (first!=null) filter = ExtractedUtils.replace(filter, "%FIRST%", escapeSearchFilter(first));
            if (aff!=null) filter = ExtractedUtils.replace(filter, "%AFFILIATION%", escapeSearchFilter(aff));
         } else {
            // simple search
            ...
            filter = ExtractedUtils.replace(filter, "%TERM%", escapeSearchFilter(searchValue));
        }
